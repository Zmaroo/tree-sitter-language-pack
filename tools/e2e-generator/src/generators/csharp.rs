    ...
    for fixture in fixtures {
        ...
        if assertions.is_some_and(|a| a.expect_error == Some(true)) {
            ...
            if has_chunk_assertions(fixture) {
                ...
                writeln!(
                    out,
                    "        var config = \"{{\\\"language\\\":\\\"{}\\\",\\\"chunk_max_size\\\":{}}}\";",
                    escape_csharp_string(lang),
                    max_chunk_size
                )
                .unwrap();
            ...
            writeln!(
                out,
                "        var resultJson = TsPackClient.Process(\"{}\", config);",
                escape_csharp_string(source)
            )
            .unwrap();
            writeln!(out, "        var intel = JsonDocument.Parse(resultJson).RootElement;").unwrap();

            if let Some(expected_lang) = &assertions.intel_language {
                writeln!(
                    out,
                    "        Assert.Equal(\"{}\", intel.GetProperty(\"language\").GetString());",
            ...
            if let Some(min_structures) = assertions.intel_structure_count_min {
                writeln!(
                    out,
                    "        Assert.True(intel.GetProperty(\"structure\").GetArrayLength() >= {}, \"Should have at least {} structure(s)\");",
            ...
            if let Some(expected_kind) = &assertions.intel_structure_contains_kind {
                writeln!(out, "        var foundKind = false;").unwrap();
                writeln!(
                    out,
                    "        foreach (var s in intel.GetProperty(\"structure\").EnumerateArray())"
                )
                .unwrap();
                writeln!(out, "        {{").unwrap();
                writeln!(
                    out,
                    "            if (s.GetProperty(\"kind\").GetString() == \"{}\") {{ foundKind = true; break; }}",
                ...
                writeln!(
                    out,
                    "        Assert.True(foundKind, \"Structure should contain a '{}' kind node\");",
                    escape_csharp_string(expected_kind)
                )
                .unwrap();
            }
            ...
            if let Some(min_imports) = assertions.intel_imports_count_min {
                writeln!(
                    out,
                    "        Assert.True(intel.GetProperty(\"imports\").GetArrayLength() >= {}, \"Should have at least {} import(s)\");",
            ...
            if assertions.intel_metrics_total_lines_min.is_some() || assertions.intel_metrics_error_count.is_some() {
                writeln!(out, "        var metrics = intel.GetProperty(\"metrics\");").unwrap();

                if let Some(min_lines) = assertions.intel_metrics_total_lines_min {
                    writeln!(
                        out,
                        "        Assert.True(metrics.GetProperty(\"total_lines\").GetInt32() >= {}, \"Should have at least {} total line(s)\");",
                        min_lines, min_lines
                    )
                    .unwrap();
                }
                ...
                if let Some(expected_error_count) = assertions.intel_metrics_error_count {
                    writeln!(
                        out,
                        "        Assert.Equal({}, metrics.GetProperty(\"error_count\").GetInt32());",
            ...
            if assertions.intel_diagnostics_not_empty == Some(true) {
                writeln!(
                    out,
                    "        Assert.True(intel.GetProperty(\"diagnostics\").GetArrayLength() > 0, \"Diagnostics should not be empty\");"
                )
                .unwrap();
            }
                ...
                && let Some(min_chunks) = assertions.intel_chunk_count_min
            {
                writeln!(
                    out,
                    "        Assert.True(intel.GetProperty(\"chunks\").GetArrayLength() >= {}, \"Should have at least {} chunk(s)\");",
