...
pub(crate) fn resolve_call_ref(ctx: &CallResolutionContext<'_>, call_ref: &CallRef) -> CallResolution {
    ...
    if let Some(filtered) = policy::resolve_call_ref_filters(ctx, call_ref) {
        return filtered;
    }
    ...
    {
        let (payload_member, init_member, container, regex, py_test, ext_py_module) =
            policy::python_scoped_trace_flags(ctx, call_ref).unwrap_or((false, false, false, false, false, false));
        eprintln!(
