...
pub fn run_query_in_byte_range_profiled(
    ...
    let query = compiled_query(language, query_source)?;
    collect_query_matches_profiled(tree, &query, source, Some(byte_range))
}
...
fn compiled_query(language: &str, query_source: &str) -> Result<Arc<tree_sitter::Query>, Error> {
    ...
    let query = Arc::new(tree_sitter::Query::new(&lang, query_source).map_err(|e| Error::QueryError(format!("{e}")))?);
